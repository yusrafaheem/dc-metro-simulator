...
public class TransferBugAndTripLengthTests {
    ...
    @Test
    public void test_a_station_beyond_metro_center_is_reachable_again_even_from_its_own_line() {
        // McPherson Square and Federal Triangle are both on the orange
        // line, one stop apart from Metro Center in opposite directions --
        // they should be 2 stops apart from each other, and now are.
        // Federal Triangle's connection to Metro Center was overwritten by
        // the red line's wiring and was never recorded in otherStations at
        // all (a separate gap in MetroSimulator's own wiring, fixed
        // alongside the traversal bug) -- both fixes together make this
        // trip work.
        MetroSimulator.initialize();
        MetroSimulator.makeOrangeLine();
        MetroSimulator.makeRedLine();
        ...
        assertEquals(2, MetroSimulator.mcpherson_square.tripLength(MetroSimulator.federal_triangle));
    }
    ...
    @Test
    public void test_transfer_station_tripLength_now_consults_otherStations_to_find_a_reachable_station() {
        // Minimal, isolated repro of the fix: TransferStation now overrides
        // getTransferNeighbors() to expose otherStations, and
        // tripLengthHelper() walks it -- a station sitting in that list
        // with no next/prev wiring of its own is a single, direct hop away.
        TransferStation t = new TransferStation("pink", "Hub");
        Station reachableViaOtherStations = new Station("pink", "Ghost");
        t.addTransferStationPrev(reachableViaOtherStations);
        assertEquals(1, t.otherStations.size());
        assertEquals(1, t.tripLength(reachableViaOtherStations));
    }

    @Test
    public void test_metro_centers_final_otherStations_list_accumulates_all_three_lines_transfer_points() {
        // otherStations is never overwritten -- by the time all three lines
        // are built, it holds one entry per addTransferStationPrev/Next
        // call across the whole construction, in call order. Federal
        // Triangle is included here too, now that makeOrangeLine() records
        // it (previously the only orange-line connection missing an
        // otherStations entry -- a separate gap fixed alongside the
        // traversal bug).
        MetroSimulator.initialize();
        MetroSimulator.makeOrangeLine();
        MetroSimulator.makeRedLine();
        ...
        assertEquals(6, MetroSimulator.metro_center.otherStations.size());
        assertSame(MetroSimulator.mcpherson_square, MetroSimulator.metro_center.otherStations.get(0));
        assertSame(MetroSimulator.federal_triangle, MetroSimulator.metro_center.otherStations.get(1));
        assertSame(MetroSimulator.farragut_north, MetroSimulator.metro_center.otherStations.get(2));
        assertSame(MetroSimulator.gallery_place, MetroSimulator.metro_center.otherStations.get(3));
        assertSame(MetroSimulator.s3, MetroSimulator.metro_center.otherStations.get(4));
        assertSame(MetroSimulator.s4, MetroSimulator.metro_center.otherStations.get(5));
    }
    ...
    @Test
    public void test_the_same_shaped_trip_on_the_red_line_through_metro_center_now_works_too() {
        // Same "two stops out, through Metro Center" shape as the
        // purple-line trip above -- red was overwritten by purple, but
        // since Metro Center now consults otherStations, the identical
        // pattern on the red line returns a real stop count too, not just
        // the line that happened to be wired last.
        MetroSimulator.initialize();
        MetroSimulator.makeOrangeLine();
        MetroSimulator.makeRedLine();
        MetroSimulator.makePurpleLine();

        assertEquals(3, MetroSimulator.dupont_circle.tripLength(MetroSimulator.gallery_place));
    }
    ...
    @Test
    public void test_virginia_square_is_seven_stops_from_metro_center_and_metro_center_can_now_reach_back() {
        // A long, real trip number that always computed correctly in one
        // direction (Virginia Square's own chain of next pointers down to
        // Metro Center was never touched), now paired with the same,
        // correct answer in the other direction too.
        MetroSimulator.initialize();
        MetroSimulator.makeOrangeLine();
        MetroSimulator.makeRedLine();
        MetroSimulator.makePurpleLine();
        ...
        assertEquals(7, MetroSimulator.metro_center.tripLength(MetroSimulator.va_square));
    }
    ...
    @Test
    public void test_a_lookalike_destination_matches_the_real_reachable_station_symmetrically() {
        // Contrast with the earlier "lookalike destination" test, which
        // showed equals()-based matching finds a real, reachable station
        // just as well as the original object -- that's still true here.
        // Previously this pair was on Metro Center's broken branch and both
        // came back -1; now that the transfer bug is fixed, both the
        // lookalike and the real object resolve to the same, correct
        // 1-stop trip.
        MetroSimulator.initialize();
        MetroSimulator.makeOrangeLine();
        MetroSimulator.makeRedLine();
